...
.metabulate <- function(meta_tables, show_msd = TRUE, show_conf = TRUE, show_cred = TRUE,
        ...
        .initial_formatting <- function(ma_table, footnote) {


                ma_table <- ma_table %>%
                  add_colnames() %>%
                  set_bold(1, everywhere, TRUE) %>%
                  set_bottom_border(c(1, nrow(ma_table)), everywhere, TRUE) %>%
                  set_align(everywhere, (length_initial + length_moderators +1):ncol(ma_table), "center") %>%
                  set_width(.80) %>%
                  set_right_padding(5) %>%
                  set_left_padding(5) %>%
                  set_caption("Results of meta-analyses") %>%
                  add_footnote(footnote)

        }

        # Apply initial formatting to the tables
        footnote_list <-
          if(es_type == "r") {
            list(bb     = paste("" sep = " "),
                 ic_ts  = "Results of meta-analyses (true score correlations, individually-corrected)",
                 ic_vgx = "Results of meta-analyses (operational validities, X measured with error, individually-corrected)",
                 ic_vgy = "Results of meta-analyses (operational validities, Y measured with error, individually-corrected)",
                 ad_ts  = "Results of meta-analyses (true score correlations, corrected using artifact distributions)",
                 ad_vgx = "Results of meta-analyses (operational validities, X measured with error, corrected using artifact distributions)",
                 ad_vgy = "Results of meta-analyses (operational validities, Y measured with error, corrected using artifact distributions)",
            )[ma_type]
          } else if(es_type == "d") {
            meta_tables <- list(bb = meta_tables$barebones,
                                ic_ts  = meta_tables$individual_correction$latentGroup_latentY,
            ...
            meta_tables <- "Results of meta-analyses (barebones)",
          }

        meta_tables <- sapply(names(meta_tables), function(x) .initial_formatting(meta_tables[[x]], x))

        # Bold header rows
        meta_tables <- lapply(meta_tables, set_bold)
        ...
        if(length_moderators > 1 | (show_conf & conf_format == "columns") | (show_cred & cred_format == "columns")) {
                ...
                if(length_moderators > 1) {
                        ...
                        meta_tables <- lapply(meta_tables, .moderator_span)
        ...
        meta_tables <- lapply(meta_tables, )
