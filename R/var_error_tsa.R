...
.var_error_r_bvirr <- function(rxyi, var_e = NULL, ni, na = NA,
     ...
     ## Partial derivatives corresponding to the multiplicative part of the correction
     ## With respect to qxa
     b_qxa <- -(rxyi * ux * uy) / (qxa^2 * qya)
     ## With respect to qya
     b_qya <- -(rxyi * ux * uy) / (qxa * qya^2)
     ## With respect to ux
     b_ux <- (rxyi * uy) / (qxa * qya)
     ## With respect to uy
     b_uy <- (rxyi * ux) / (qxa * qya)
     
     var_mult <- as.numeric(b_qxa^2 * var_e_qxa + b_qya^2 * var_e_qya + b_ux^2 * var_e_ux + b_uy^2 * var_e_uy + b_rxyi^2 * var_e)
     
     
     ## Partial derivatives corresponding to the additive part of the correction
     ## With respect to qxa
     b_qxa <- -(sqrt(abs(1 - ux^2)) * sqrt(abs(1 - uy^2))) / (qxa^2 * qya)
     ## With respect to qya
     b_qya <- -(sqrt(abs(1 - ux^2)) * sqrt(abs(1 - uy^2))) / (qxa * qya^2)
     ## With respect to ux
     b_ux <- -(ux * (1 - ux^2) * sqrt(abs(1 - uy^2))) / (qxa * qya * abs(1 - ux^2)^(3/2))
     ## With respect to uy
     b_uy <- -(uy * (1 - uy^2) * sqrt(abs(1 - ux^2))) / (qxa * qya * abs(1 - uy^2)^(3/2))
     
     ## Re-estimate any undefined derivatives
     if(any(is.na(b_ux))){
          b_ux[is.na(b_ux)] <- (-(ux * (1 - ux^2) * sqrt(abs(1 - uy^2))) / (qxa * qya * abs(1 - ux^2 + .01)^(3/2)))[is.na(b_ux)]
     }
     if(any(is.na(b_uy))){
          b_uy[is.na(b_uy)] <- (-(uy * (1 - uy^2) * sqrt(abs(1 - ux^2))) / (qxa * qya * abs(1 - uy^2 + .01)^(3/2)))[is.na(b_uy)]
     }
     
     var_add <- as.numeric(b_qxa^2 * var_e_qxa + b_qya^2 * var_e_qya + b_ux^2 * var_e_ux + b_uy^2 * var_e_uy)
     
     data.frame(var_mult = var_mult, 
                var_add = var_add)
}
