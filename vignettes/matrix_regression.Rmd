  ...
  - `se_beta_method`. This argument controls how standard errors and confidence 
    ...
    [not covariance] matrix). 
      - The default, `"lm"`, will compute standard errors using the same method 
        as `lm()`, based on @Cohen2003AppliedMultiple. This method ignores uncertainty 
      ...
      - More accurate standard errors and confidence intervals for standardized 
        ...
        which will use the normal-theory approach from @JonesComputingconfidenceintervals2013.
...
```{r}
print(meta_matrix$individual_correction$`1`$true_score)
```
...
```{r}
R_meta <- meta_matrix$individual_correction$`1`$true_score$mean_rho
```
...
mean *N* across meta-analyses is often used [@ViswesvaranTheoryTesting1995]:
...
N_meta <- meta_matrix$individual_correction$`1`$true_score$N
...
results, consider a GLS approach based on @BeckerUsingresultsreplicated1992. 
Functions for this approach will be added in a future version.**
