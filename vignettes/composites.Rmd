...
This vignette shows options for computing composite effect sizes in **psychmeta**.
For more vignettes, see the [**psychmeta** overview](overview.html).

**Note:** When a meta-analysis includes dependent effect sizes (indicated by
the `sample_id` argument in `ma_r()` or `ma_d()`), by default **psychmeta** will
automatically compute composite effect sizes and artifacts, assuming an average
correlation among dependent measures of _r_ = .50. The functions described below
can be used to compute the composite effect sizes and artifacts yourself.
For more information, see [Effect size dependency](dependency.html).
